...
long i2_handle_position_lost_in_2(short a, short b, short c, short d, short e, short f, short g, short h, FILE * fp, short undoneMove) {
	long position;
...
long i2_handle_position_won_in_1(short a, short b, short c, short d, short e, short f, short g, short h, FILE * fp, short undoneMove) {
	long count = 0;
	short tmp, org, dst;
	...
	turn_board()
	undo_all_moves(
		// In here every position is lost no matter what
		count += i2_handle_position_lost_in_2(a, b, c, d, e, f, g, h, fp, undoneMove);
	)
	return count;
}

long i2_handle_won_position(short a, short b, short c, short d, short e, short f, short g, short h, FILE * fp) {
	long count = 0;
	short org, dst, undoneMove;
	undo_all_win_moves(
		// In here every position is won with one ply
		count += i2_handle_position_won_in_1(a, b, c, d, e, f, g, h, fp, undoneMove);
	)
	return count;
}

// All 4 seeds find 747.487.398 results
void * iteration2(void * seedPointer) {
	...
	generate_won_positions(seed,
		count += i2_handle_won_position(a, b, c, d, e, f, g, h, fp);
	)
